...
public class ShpImport {
	...
	private static GeometryLayer getLayerFromPolygonShp(ShapeFile shp, Context context) {
		...
		for(int i=0;i<shp.getSHP_shapeCount() ; i++){
			...
			for(int j = 0 ; j < polygon.getNumberOfPoints();j++){
				p.addPoint(new Point(points[j][1], points[j][0]));
	...
	private static GeometryLayer getLayerFromPointShp(ShapeFile shp, Context context){
	...
	private static GeometryLayer getLayerFromPolylineShp(ShapeFile shp, Context context){
		...
		for(int i=0;i<shp.getSHP_shapeCount() ; i++){
			...
			for(int j = 0 ; j < shpline.getNumberOfPoints();j++){
				line.addPoint(new Point(points[j][1], points[j][0]));
			}
			gl.addGeometry(line);
		}
		return gl;
